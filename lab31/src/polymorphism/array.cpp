...
void BOOK_ARRAY::read_from_file( BOOK *sf, BOOK *ab, string path)
{
    ifstream fin(path);
    ...
    else
    {
		...
		{	
			...
			{
				ab->from_string(tmp_result);
				cout << tmp_result << endl;
				array.push_back( ab );
			}
				//array[i].push_back( ab->from_string(tmp_result) );
...
struct sort_books  //функтор сортировки
{
...
void BOOK_ARRAY::SortByPages()
{
    sort (array.begin(), array.end(), SortPage);  //сортировка с какого по какой элемент
}
